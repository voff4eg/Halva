 ...
 * Подключение Route
 */
require_once $_SERVER["DOCUMENT_ROOT"] . '/bitrix/modules/geo_timedelivery/classes/general/router/includeAll.php';
...
$city = iconv('UTF-8', 'CP1251', $arParams['CITY']); //htmlspecialchars(iconv('UTF-8', 'CP1251', $_REQUEST['city']));
$street = iconv('UTF-8', 'CP1251', $arParams['STREET']); //htmlspecialchars(iconv('UTF-8', 'CP1251', $_REQUEST['street']));
$number_home = intval($arParams['NUMBER_HOME']); //intval($_REQUEST['number_home']);
$restaurant_id = intval($arParams['RESTAURANT_ID']); //intval($_REQUEST['restaurant_id']);
$delivery_type = iconv('UTF-8', 'CP1251', $arParams['DELIVERY_TYPE']); //htmlspecialchars(iconv('UTF-8', 'CP1251', $_REQUEST['delivery_type']));

$addressStart = $city . ' ' . $street . ' ' . $number_home; // Адрес клиента
$restaurantId = $restaurant_id; // id ресторана
$deliveryType = $delivery_type; // Тип доставки
...
if ($deliveryType == 'walking') {
	...
	try {
		$route = array();
		$r = $obAddressAddressRouter->build(cp1251_to_utf8($arRestaurant['PROPERTY_ADDRESS_VALUE']), iconv('CP1251', 'UTF-8', $addressStart));
		$route['time'] = $r['time'];
...
//	print_r(array($arRestaurant['PROPERTY_ADDRESS_VALUE'], $addressStart));
	$r = $obAddressAddressRouterWalking->build(iconv('CP1251', 'UTF-8', $city . ' ' . $arRestaurant['PROPERTY_ADDRESS_VALUE']), iconv('CP1251', 'UTF-8', $addressStart));
	try {
		$routeWalking = array();
		...
		$routeWalking['time'] = 0;
		$routeWalking['instruction'] = iconv('UTF-8', 'CP1251', $e->getMessage());
		$routeWalking['route'] = '';
	}
	
//	print_r($routeWalking);

	$obAddressAddressRouterDriving = new AddressAddressRouter('driving', 'AIzaSyDpQVm5bs0BJkQGPr17KfclXphDtBCvwHg');
	try {
		$routeDriving = array();
		$r = $obAddressAddressRouterDriving->build(iconv('CP1251', 'UTF-8', $arRestaurant['PROPERTY_ADDRESS_VALUE']), iconv('CP1251', 'UTF-8', $addressStart));
